...
item.gps (Generation per second) Количество обработанных поколений в секундe
...
def statistic(dimension):
    ...
    for line in dimension:
        ...
        avgCalculatedCell = sum([((item.resolution/10) ** 2) * item.numberGenerations for item in line]) / len(line)
...
if __name__ == '__main__':
    #runBenchMark(10,1)
    game = GameOfLife.GameOfLife(500, 500, 5, speed=10000)
    generationCount = game.run()
